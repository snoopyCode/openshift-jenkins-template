...
def qqUserName = null
def qqUserPassword = null

if (env["TARGET_ENVIRONMENT"] == "TEST") {
    qqUserName = env["QQ_USER_NAME"]?.trim()
    qqUserPassword = env["QQ_USER_PASSWORD"]?.trim()
}
...
if (env["TARGET_ENVIRONMENT"] == "TEST") {
    ...
    println("Skipping creation of job \"DependencyCheck_DataUpdate\": Not in target environment.")
}
...
if (env["TARGET_ENVIRONMENT"] == "TEST") {
    ...
    println("Skipping configuration of stashNotifier (Bitbucket Server): Not in target environment.")
}
...
if (env["TARGET_ENVIRONMENT"] == "TEST") {
    ...
    println("Skipping configuration of jira-steps: Not in target environment.")
}